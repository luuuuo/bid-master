...
describe("BlindAuctionPro test", function () {
  ...
  async function deployBlindFixture() {
    const { auctionFactory } = await deployAuctionFactory();
    ...
    await auctionFactory.createAuctions(0, blindAuctionLogic.address);
  ...
  async function deployOpenFixture() {
    ...
    await auctionFactory.createAuctions(1, openAuctionLogic.address);
